...
def main(fp_in, fp_out, is_group_repeated, num_atoms_in_group, index_to_filtered):
    region = 'head'
    for line in fp_in:
        if region == 'head':
            ...
            fp_out.write(NUM_ATOMS_TEMPLATE % (num_atoms / num_atoms_in_group * len(index_to_filtered)))
            atom_index = 0
            region = 'body'
        elif region == 'body':
            assert(atom_index < num_atoms)
            if atom_index % num_atoms_in_group in index_to_filtered:
                fp_out.write(line)
            atom_index += 1
    ...
    with open(path) as fp:
        for line in fp:
            ...
            if len(line) == 0 or line[0] == '#':  # Comment line.
                ...
                index_to_filtered[int(ss[0]) - 1] = True  # Convert 1-origin to 0-origin. True is a dummy value.
    return num_atoms_in_group, index_to_filtered

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='')
    parser.add_argument('input_gro_path', metavar='GRO', type=str,
                        help='')
    ...
    num_atoms_in_group, index_to_filtered = read_group_id(args.group_id_path, args.group_index)
    if args.output_path is None:
        output_path = re.sub('\.[^.]+$', '', args.input_gro_path) + '_filter.gro'
    else:
        output_path = args.output_path
    print 'in: ', args.input_gro_path
    ...
    with open(args.input_gro_path, 'r') as fp_in, open(output_path, 'w') as fp_out:
        main(fp_in, fp_out, args.is_group_repeated, num_atoms_in_group, index_to_filtered)
